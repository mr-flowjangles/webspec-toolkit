...
- `packages/chrome-extension/` — **The v1 primary surface.** Manifest V3. Bundles the browser flavor of `core`: `A11yAnalyzer` (browser mode), `WorkflowRecorder`, `ReportRenderer`, and the **deterministic** `E2ERenderer` (`renderPlaywrightSpec` is browser-safe; the LLM amplifier is not and lives in Node only). On Save, the extension renders the spec in-browser and writes the file pair via `chrome.downloads`. **Does not** bundle `TestPlanAnalyzer` (no filesystem access) or the LLM amplifier (AWS/Anthropic SDKs are Node-only). v1.2+ direction (`docs/08-test-library.md`): Save lands files in a per-test slug folder under `~/Downloads/webspec/` and writes a parent `playwright.config.ts` so `playwright test --ui` discovers them — Playwright UI is the see-and-execute surface; the extension stays an authoring surface.
