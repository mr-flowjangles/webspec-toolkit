...
**Live on `main` (v1.2.0):** v1.1.0 user-supplied test name + description + dual-file Save; v1.1.1 `.spec.ts` MIME fix; v1.1.2 test-library design doc (`docs/08-test-library.md`); v1.2.0 on-disk test library at `~/Downloads/webspec/<slug>/` with parent `playwright.config.ts` discovered by Playwright UI via `make run-tests`. The extension is the authoring surface; Playwright UI is the see-and-execute surface (we deliberately don't build an in-extension library list — see [[m1-3-pivot]] in memory).

**In flight on branch `V1dot3dot0/Domain-Aware_Auth_Profiles` (PR #41):** v1.3.0 — domain-aware auth profiles. New ⚙ Settings tab manages `AuthProfile`s (name + URL glob + N header rows with `${runAs}` substitution) stored in `chrome.storage.local`. Popup matches the active tab URL against profiles at record-start; the naming form shows the matched profile. Resolved headers are baked into `WorkflowRecording.auth` and emitted by the renderer as `await context.setExtraHTTPHeaders({ ... })` — same mechanism as ModHeader. **Also folded into PR #41:** selector hardening (`packages/chrome-extension/src/content-script/selectors.ts`) — clicks landing on decorative descendants of interactive elements (mat-icon inside `[role=menuitem]`, etc.) now promote up to the nearest interactive ancestor before hardening, replacing brittle `div >> nth=369` selectors with stable `getByRole(...)`. PR awaits Rob's manual smoke against UCM (see [[resume-here-next-session]]).
...
**Post-v1 design (only v1.4 not yet implemented).** `docs/08-test-library.md` is authoritative for the v1.2 → v1.4 stack. v1.2 and v1.3 are coded; v1.4 (suites — `suite.json` + `test.describe.serial` rendering, CLI/Makefile suite creation) remains designed but not built.
